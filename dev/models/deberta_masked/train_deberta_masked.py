...
# NLTK imports for sense retrieval
import nltk
nltk.data.path.append("./nltk_data")
from nltk.corpus import wordnet as wn


# --- 1. Custom Dataset Class with Logit Masking ---
class WSDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Handle multi-word expressions by replacing spaces with underscores for NLTK lookup
        ...
        # 4. Ensure the true label is unmasked
        # This prevents NaN loss if NLTK misses the specific sense.
        mask[label_id] = 0.0
...
class DebertaV3ForWSD(DebertaV2PreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, target_token_idx=None, labels=None, logit_mask=None, **kwargs):
        ...
        # --- Apply Logit Masking ---
        # Add the mask to logits. 
        # Valid senses: logits + 0 = logits
        # Invalid senses: logits + (-10000) = very small number
...
def main():
    parser = argparse.ArgumentParser(description="Train DeBERTa V3 for WSD with Logit Masking")
    ...
    parser.add_argument("--output_dir", type=str, default="./deberta_wsd_masked", help="Directory to save model")
    ...
    # Pass label2id to dataset for masking logic
    train_dataset = WSDDataset(train_df, tokenizer, label2id)
    val_dataset = WSDDataset(val_df, tokenizer, label2id)
    ...
    def compute_metrics(eval_pred):
        ...
        # Logits already contain the mask effect here because it was applied in forward()
    ...
    training_args = TrainingArguments(
        ...
        remove_unused_columns=False # Otherwise 'logit_mask' might be dropped by Trainer
    )
    ...
    # Train
    print("Starting training with Logit Masking...")
    trainer.train()
